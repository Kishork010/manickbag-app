...
const PageStyles = () => (
  <style>{`
    ...
    .eb-fleet-card { transition:all 0.3s ease; cursor:pointer; }
    .eb-fleet-card:hover { transform:translateY(-4px); box-shadow:0 20px 48px rgba(0,0,0,0.1); }
    .eb-fleet-card.selected { border-color:#b8963e !important; background:#0c1f3f !important; }
    ...
    .eb-input { width:100%; padding:12px 16px; background:rgba(255,255,255,0.07); border:1px solid rgba(184,150,62,0.2); color:#ffffff; font-family:'Jost',sans-serif; font-size:13px; outline:none; border-radius:2px; transition:border-color 0.2s; }
    .eb-input:focus { border-color:#b8963e; }
    .eb-input::placeholder { color:rgba(255,255,255,0.3); }

    .eb-select { width:100%; padding:12px 16px; background:rgba(255,255,255,0.07); border:1px solid rgba(184,150,62,0.2); color:#ffffff; font-family:'Jost',sans-serif; font-size:13px; outline:none; border-radius:2px; cursor:pointer; }
...
const acceptedBrands = ["Maruti Suzuki","Hyundai","Honda","Mahindra","Kia","Toyota","Renault","Volkswagen","Skoda","MG Motor","Ford","Nissan","Jeep","Tata Motors"];
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:24 }}>
            {exchangeSteps.map((step,i) => (
              <div key={step.num} style={{ textAlign:"center", position:"relative", animation:`eb-fadeUp 0.5s ease ${i*0.1}s both` }}>
                {i < exchangeSteps.length-1 && <div style={{ position:"absolute", top:36, left:"65%", width:"70%", height:1, background:`linear-gradient(90deg,${BRAND.gold},transparent)`, zIndex:0 }} />}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:48 }}>
            <div style={{ background:BRAND.navyMid, padding:"40px 36px" }}>
              ...
              {submitted ? (
                <div style={{ textAlign:"center", padding:"36px 0" }}>
                  ...
                  <div style={{ fontSize:13, color:"rgba(255,255,255,0.5)", lineHeight:1.8, marginBottom:20 }}>Our exchange specialist will call <strong style={{ color:BRAND.gold }}>{formData.phone}</strong> within 2 hours.</div>
                  {formData.newModel && (
                  ...
                  <button className="eb-btn-outline" onClick={() => { setSubmitted(false); setFormData({ name:"",phone:"",oldBrand:"",oldModel:"",oldYear:"",oldKm:"",newModel:"",city:"" }); setSelectedNew(null); }}
                    style={{ padding:"12px 28px", fontSize:12, borderRadius:2 }}>New Enquiry</button>
                </div>
              ) : (
                <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                  ...
                  {[["name","Your Name","text","Full name"],["phone","Mobile Number","tel","+91 98765 43210"]].map(([k,l,t,p]) => (
                    <div key={k}>
                  ...
                  {[["oldModel","Model Name","text","e.g. Swift, Creta"],["oldYear","Year of Purchase","text","e.g. 2019"],["oldKm","Kilometres Driven","text","e.g. 45,000 km"]].map(([k,l,t,p]) => (
                    <div key={k}>
                      <label style={{ display:"block", fontSize:10, letterSpacing:"0.1em", color:"rgba(255,255,255,0.4)", textTransform:"uppercase", marginBottom:5 }}>{l}</label>
                      <input type={t} placeholder={p} className="eb-input" value={formData[k]} onChange={e => updateForm(k,e.target.value)} />
                    </div>
                  ...
                  <div>
                    ...
                    <select className="eb-select" value={formData.city} onChange={e => updateForm("city",e.target.value)}>
                      ...
                      {["Belgaum","Hubbli","Dharwad","Karwar","Bijapur","Gulbarga","Bidar","Yadgiri"].map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                  </div>
                  <button className="eb-btn-gold" onClick={() => canSubmit && setSubmitted(true)}
                    style={{ padding:"14px", fontSize:12, borderRadius:2, marginTop:4, opacity:canSubmit?1:0.5 }}>
                    Get My Exchange Quote →
                  </button>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div style={{ background:BRAND.offWhite, padding:"24px" }}>
                {[["📄","Original RC Book","Registration Certificate — original required"],["🛡️","Valid Insurance","Current policy copy"],["🌿","PUC Certificate","Valid Pollution Under Control cert"],["🪪","Photo ID","Aadhaar / PAN / Passport"],["📋","Service Records","All available service history booklets"],["🔑","All Keys","Both sets of vehicle keys"]].map(([icon,title,desc]) => (
      ...
      <div style={{ background:`linear-gradient(135deg,${BRAND.navy},${BRAND.navyLight})`, padding:"68px 48px" }}>
        <div style={{ ...W, padding:0, display:"grid", gridTemplateColumns:"1fr 1fr", gap:56, alignItems:"center" }}>
          ...
          <div style={{ display:"flex", gap:16, flexWrap:"wrap", justifyContent:"flex-end" }}>
            <button className="eb-btn-gold" style={{ padding:"16px 36px", fontSize:13, borderRadius:2 }}>📍 Find Showroom</button>
            <button className="eb-btn-outline" style={{ padding:"16px 36px", fontSize:13, borderRadius:2 }}>📞 Call Exchange Desk</button>
          </div>
        </div>
      </div>
    </Layout>
