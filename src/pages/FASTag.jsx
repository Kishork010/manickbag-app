...
const PageStyles = () => (
  <style>{`
    ...
    .ft-btn-gold { background:linear-gradient(135deg,#b8963e,#d4af5a); color:#0a1628; border:none; cursor:pointer; font-family:'Jost',sans-serif; font-weight:600; letter-spacing:0.12em; text-transform:uppercase; transition:all 0.3s; }
    .ft-btn-gold:hover { opacity:0.9; transform:translateY(-1px); }

    .ft-btn-outline { background:transparent; border:1px solid #b8963e; color:#b8963e; cursor:pointer; font-family:'Jost',sans-serif; font-weight:500; letter-spacing:0.1em; text-transform:uppercase; transition:all 0.3s; }
...
const faqs = [
  ...
  { q:"How much does FASTag cost at Manickbag?",                      a:"The FASTag tag costs ₹100 (one-time). A minimum security deposit of ₹200 and ₹100 initial load is required — total ₹400 to get started." },
  ...
  { q:"What if my vehicle passes a toll with insufficient FASTag balance?", a:"The toll operator will charge double the toll fee in cash. Always keep a minimum balance of ₹200 to avoid penalties." },
];

export default function FASTag() {
  const [openFaq,   setOpenFaq]   = useState(null);
  const [formData,  setFormData]  = useState({ name:"", phone:"", regNo:"", vehicle:"" });
  const [submitted, setSubmitted] = useState(false);

  const handleSubmit = () => {
    if (formData.name && formData.phone && formData.regNo) setSubmitted(true);
  };
      ...
      <div style={{ maxWidth:1280, margin:"0 auto", padding:"64px 48px" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:20, marginBottom:64 }}>
          {steps.map((step,i) => (
            <div key={step.num} className="ft-step-card"
              ...
              {i < steps.length-1 && <div style={{ position:"absolute", right:-20, top:"50%", transform:"translateY(-50%)", color:BRAND.gold, fontSize:20, zIndex:1 }}>›</div>}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:40, marginBottom:64 }}>
          <div style={{ background:BRAND.navyMid, padding:"40px 36px" }}>
            ...
            {submitted ? (
              <div style={{ textAlign:"center", padding:"40px 0" }}>
                ...
                <div style={{ fontSize:13, color:"rgba(255,255,255,0.5)" }}>Our team will contact you within 2 hours to schedule your FASTag appointment.</div>
              </div>
            ) : (
              <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                {[["name","Full Name","text","Enter your name"],["phone","Mobile Number","tel","+91 XXXXX XXXXX"],["regNo","Vehicle Registration No.","text","KA-XX-XXXX"],["vehicle","Vehicle Model","text","e.g. Tata Nexon"]].map(([field,label,type,placeholder]) => (
                  <div key={field}>
                    <label style={{ display:"block", fontSize:11, letterSpacing:"0.1em", color:BRAND.gold, textTransform:"uppercase", marginBottom:6 }}>{label}</label>
                    <input type={type} placeholder={placeholder} value={formData[field]}
                      onChange={e => setFormData(d => ({ ...d, [field]:e.target.value }))}
                      style={{ width:"100%", padding:"12px 16px", background:"rgba(255,255,255,0.07)", border:`1px solid rgba(184,150,62,0.2)`, color:BRAND.white, fontFamily:"'Jost',sans-serif", fontSize:13, outline:"none", borderRadius:2 }} />
                  </div>
                ))}
                <button className="ft-btn-gold" onClick={handleSubmit} style={{ padding:"14px", fontSize:13, borderRadius:2, marginTop:8 }}>Submit Enquiry →</button>
              </div>
            )}
          </div>
          ...
          <div>
            <div style={{ background:BRAND.offWhite, padding:"32px", marginBottom:20 }}>
              ...
              {[["📄","Vehicle RC","Original + Photocopy"],["🪪","Photo ID","Aadhaar / PAN / Passport"],["📸","Passport Photo","1 recent photograph"],["📱","Mobile Number","Linked to bank account"]].map(([icon,doc,detail]) => (
            ...
            <div style={{ background:BRAND.navyMid, padding:"28px 32px" }}>
              ...
              {[["FASTag Tag Fee","₹100",false],["Security Deposit","₹200",false],["Minimum Load","₹100",false],["Total to Pay","₹400",true]].map(([item,amt,bold]) => (
              ...
              <div style={{ marginTop:16, fontSize:10, color:"rgba(255,255,255,0.3)", lineHeight:1.6 }}>* Security deposit refundable on account closure. Tag fee non-refundable.</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ background:BRAND.offWhite, padding:"40px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:12 }}>
            {banks.map(b => (
              <div key={b.name} style={{ padding:"16px 12px", border:"1px solid rgba(0,0,0,0.06)", textAlign:"center", background:BRAND.white, transition:"all 0.2s" }}
