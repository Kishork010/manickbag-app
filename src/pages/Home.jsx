  ...
  // ══════════════════════════════════════════════════════════════════
  const VehiclesSection = ({ sectionRef, onQuote }) => {
  ...
  <button
    style={{ flex: 1, padding: "10px", fontSize: 11, cursor: "pointer", background: BRAND.navyMid, color: BRAND.white, border: "none", fontFamily: "'Jost',sans-serif", fontWeight: 500, letterSpacing: "0.08em", textTransform: "uppercase", transition: "background 0.2s" }}
    ...
    onClick={() => navigate(`/vehicles/${v.name.toLowerCase().replace(/\s+/g, '-')}`)}
  >Explore</button>
                  <button
    style={{ padding: "10px 14px", fontSize: 11, cursor: "pointer", background: "transparent", color: BRAND.gold, border: `1px solid ${BRAND.gold}`, fontFamily: "'Jost',sans-serif", transition: "all 0.2s" }}
  ...
  export default function Home() {
    ...
    const [quoteVehicle, setQuoteVehicle] = useState(null); // ← ADD THIS
    ...
    return (
      <div style={{ minHeight: "100vh", width: "100%", background: "#ffffff", overflowX: "hidden" }}>
        ...
        <Hero />
        <Ticker />
        <VehiclesSection sectionRef={vehiclesSectionRef} onQuote={setQuoteVehicle} /> {/* ← pass onQuote */}
        <HeritageSection />
        <ServicesSection />
        ...
        <Footer />
        <FloatingWA />
        {/* QuotePopup lives HERE — outside VehiclesSection so it never re-mounts on filter changes */}
        {quoteVehicle && (
          <QuotePopup vehicleName={quoteVehicle} onClose={() => setQuoteVehicle(null)} />
        )}
      </div>
    );
  }