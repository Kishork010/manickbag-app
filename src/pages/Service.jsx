...
const PageStyles = () => (
  <style>{`
    ...
    .sp-btn-gold:disabled { opacity:0.42; cursor:not-allowed; }
    ...
    /* light inputs — popup form */
    .sp-input-light {
      width:100%; padding:11px 14px;
      background:#ffffff; border:1px solid rgba(10,31,63,0.15);
    ...
    .sp-overlay {
      ...
      padding:16px; animation:sp-fadeIn 0.25s ease; backdrop-filter:blur(4px);
    }
    .sp-popup {
    ...
    .sp-field-err { font-size:11px; color:#e53e3e; margin-top:4px; }
  `}</style>
);
...
function PackageBookingPopup({ pkg, onClose }) {
  ...
  const validate = () => {
    ...
    if (!/^\d{10}$/.test(form.phone.trim())) e.phone         = "Enter valid 10-digit mobile number";
  ...
  const handleSubmit = async () => {
    ...
    try {
      await submitServiceBooking({
        name         : form.fullName,
        phone        : form.phone,
        regNo        : form.vehicleNumber,
        model        : `${form.model}${form.variant ? " " + form.variant : ""}`,
        ...
        issues       : [
          ...
          form.mfgYear   ? `Mfg Year: ${form.mfgYear}`   : null,
          `KMs: ${form.kms}`,
      ...
      <div className="sp-popup">
        ...
        <div style={{ background:BRAND.navyMid, padding:"22px 28px", display:"flex", justifyContent:"space-between", alignItems:"center", position:"sticky", top:0, zIndex:10 }}>
          ...
          <button onClick={onClose}
            style={{ background:"rgba(255,255,255,0.08)", border:"none", color:BRAND.white, width:34, height:34, borderRadius:"50%", cursor:"pointer", fontSize:20, lineHeight:"34px", textAlign:"center", transition:"background 0.2s" }}
          ...
          {submitted ? (
            ...
            <div style={{ textAlign:"center", padding:"32px 16px" }}>
              ...
              <p style={{ fontSize:14, color:BRAND.muted, lineHeight:1.8, marginBottom:8 }}>
                We've received your <strong>{pkg.label}</strong> booking for your <strong style={{ color:BRAND.navyMid }}>{form.model}{form.variant ? " " + form.variant : ""}</strong>.
              </p>
              ...
              <div style={{ background:BRAND.offWhite, borderLeft:`4px solid ${BRAND.gold}`, padding:"16px 20px", textAlign:"left", marginBottom:24 }}>
                <div style={{ fontSize:10, letterSpacing:"0.15em", textTransform:"uppercase", color:BRAND.gold, marginBottom:10 }}>Summary</div>
                {[
                  ["Package",        `${pkg.km} — ${pkg.label}`],
                  ["Price",          pkg.price],
            ...
            <>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                ...
                <div style={{ gridColumn:"1/-1" }}>
                  <LabelRow label="Full Name" required err={errors.fullName}>
                    <input className="sp-input-light" placeholder="Enter your full name" value={form.fullName}
                ...
                <LabelRow label="Mobile Number" required err={errors.phone}>
                  <input className="sp-input-light" placeholder="10-digit number" maxLength={10} value={form.phone}
                    onChange={e => set("phone", e.target.value.replace(/\D/,""))}
                    style={{ borderColor:errors.phone?"#e53e3e":undefined }} />
                </LabelRow>

                {/* Email */}
                <LabelRow label="Email Address">
                  <input className="sp-input-light" type="email" placeholder="you@email.com" value={form.email} onChange={e=>set("email",e.target.value)} />
                </LabelRow>

                {/* Address */}
                <div style={{ gridColumn:"1/-1" }}>
                  <LabelRow label="Address / City">
                    <input className="sp-input-light" placeholder="Your area / city" value={form.address} onChange={e=>set("address",e.target.value)} />
                  </LabelRow>
                </div>
                ...
                <LabelRow label="Tata Model" required err={errors.model}>
                  {selectWrap(
                    <select className="sp-select-light" value={form.model}
                      onChange={e => { set("model", e.target.value); set("variant",""); }}
                      style={{ borderColor:errors.model?"#e53e3e":undefined }}>
                ...
                <LabelRow label="Variant">
                  {selectWrap(
                    <select className="sp-select-light" value={form.variant} onChange={e=>set("variant",e.target.value)} disabled={!form.model}>
                ...
                <LabelRow label="Manufacturing Year">
                  {selectWrap(
                    <select className="sp-select-light" value={form.mfgYear} onChange={e=>set("mfgYear",e.target.value)}>
                ...
                <LabelRow label="Vehicle Number" required err={errors.vehicleNumber}>
                  <input className="sp-input-light" placeholder="KA-XX-XXXX" value={form.vehicleNumber}
                    onChange={e=>set("vehicleNumber", e.target.value.toUpperCase())}
                    style={{ borderColor:errors.vehicleNumber?"#e53e3e":undefined }} />
                </LabelRow>

                {/* KMs */}
                <div style={{ gridColumn:"1/-1" }}>
                  <LabelRow label="Current KMs Driven" required err={errors.kms}>
                    <input className="sp-input-light" placeholder="e.g. 18,500 km" value={form.kms}
                      onChange={e=>set("kms",e.target.value)}
                      style={{ borderColor:errors.kms?"#e53e3e":undefined }} />
                  </LabelRow>
                </div>
              ...
              <div style={{ display:"flex", gap:10, marginTop:24 }}>
                <button className="sp-btn-outline" onClick={onClose} style={{ flex:1, padding:"13px", fontSize:12, borderRadius:2 }}>Cancel</button>
                <button className="sp-btn-gold" onClick={handleSubmit} disabled={loading} style={{ flex:2, padding:"13px", fontSize:12, borderRadius:2 }}>
                  {loading ? "Submitting…" : "Confirm Booking →"}
                </button>
              </div>
              <p style={{ fontSize:11, color:BRAND.muted, textAlign:"center", marginTop:10, lineHeight:1.6 }}>
...
export default function ServicePage() {
  ...
  // ── FIX: explicit Boolean checks so empty string = false ──
  ...
  const canProceed2 = Boolean(
    form.showroom !== "" &&
    form.date     !== "" &&
    form.time     !== ""
  );

  const handleNext = () => { if (canProceed1) setStep(2); };

  const handleSubmit = async () => {
    if (!canProceed2) return;
    setLoading(true);
    try {
      await submitServiceBooking({ ...form, service_type: selectedType });
      setSubmitted(true);
    } catch (err) {
      ...
      <div style={{ maxWidth:1280, margin:"0 auto", padding:"60px 48px 0" }}>
        <div style={{ textAlign:"center", marginBottom:40 }}>
          ...
          <p style={{ fontSize:13, color:BRAND.muted, marginTop:8 }}>
            Select a type below — then fill the form to book your appointment.
          </p>
        </div>
        ...
        {!selectedType && (
          <div style={{ textAlign:"center", marginBottom:48, padding:"14px 20px", background:"rgba(184,150,62,0.07)", border:`1px dashed rgba(184,150,62,0.35)`, borderRadius:2 }}>
            <span style={{ fontSize:13, color:BRAND.gold }}>
              ⬆ Select a service type above to enable the booking form
            </span>
          </div>
        )}

        {/* ── Booking Form + Process side-by-side ── */}
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                    ...
                    {[
                      ...
                      <div key={k}>
                        ...
                        <input type={t} placeholder={p} className="sp-input"
                          value={form[k]} onChange={e => updateForm(k, e.target.value)} />
                      </div>
                    ))}
                    ...
                    {/* Warning if service type not yet chosen */}
                    {!selectedType && (
                    ...
                    {/* NEXT BUTTON — uses disabled prop, not opacity trick */}
                    <button
                      className="sp-btn-gold"
                      onClick={handleNext}
                      disabled={!canProceed1}
                      style={{ padding:"13px", fontSize:12, borderRadius:2, cursor:canProceed1?"pointer":"not-allowed" }}
                    >
                      {canProceed1
                        ? "Next: Choose Slot →"
