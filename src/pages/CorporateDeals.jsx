...
const PageStyles = () => (
  <style>{`
    ...
    .cd-nav-link::after { content:''; display:block; height:1px; background:#b8963e; width:0; transition:width 0.3s ease; }
...
// ─── TICKER ───────────────────────────────────────────────────────
const Ticker = () => {
  const items = ["Corporate Pricing Available","Fleet Discounts Up to ₹60,000","Dedicated Account Manager","Priority Service Lanes","GST Benefits on Fleet Purchase","EMI from 7.99% p.a."];
...
export default function CorporateDeals() {
  ...
  const toggleModel = (name) => setSelectedModels(s => s.includes(name) ? s.filter(x => x !== name) : [...s, name]);
            ...
            {fleetModels.map((m, i) => {
              ...
              return (
                <div key={m.name}
                  ...
                  {m.popular && <div style={{ background: BRAND.gold, color: BRAND.navy, fontSize: 9, fontWeight: 700, letterSpacing: "0.2em", textAlign: "center", padding: "5px", textTransform: "uppercase" }}>⭐ Most Popular for Fleets</div>}
          ...
          {selectedModels.length > 0 && (
            <div style={{ background: BRAND.navyMid, padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", border: `1px solid ${BRAND.borderLight}` }}>
              <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                ...
                {selectedModels.map(m => <span key={m} style={{ background: "rgba(184,150,62,0.15)", border: `1px solid ${BRAND.borderLight}`, color: BRAND.gold, fontSize: 12, padding: "4px 12px", borderRadius: 2 }}>{m}</span>)}
              </div>
                ...
                <div style={{ textAlign: "center", padding: "40px 0" }}>
                  ...
                  {selectedModels.length > 0 && (
                    <div style={{ background: "rgba(184,150,62,0.1)", border: `1px solid ${BRAND.borderLight}`, padding: "16px 20px", textAlign: "left", marginBottom: 20 }}>
                      ...
                      {selectedModels.map(m => <div key={m} style={{ fontSize: 13, color: "rgba(255,255,255,0.7)", padding: "4px 0" }}>› {m}</div>)}
                    </div>
                  )}
                  <button className="cd-btn-outline" onClick={() => { setSubmitted(false); setFormData({ company:"",name:"",phone:"",email:"",gst:"",fleetSize:"",city:"" }); }} style={{ padding: "12px 28px", fontSize: 12, borderRadius: 2 }}>Submit Another Enquiry</button>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  {[["company","Company / Organisation Name","text","ABC Pvt Ltd"],["name","Contact Person Name","text","Your name"],["phone","Mobile Number","tel","+91 98765 43210"],["email","Business Email","email","name@company.com"],["gst","GST Number (Optional)","text","29XXXXX1234Z1ZX"]].map(([k,l,t,p]) => (
                    <div key={k}>
                      ...
                      <input type={t} placeholder={p} className="cd-input" value={formData[k]} onChange={e => updateForm(k, e.target.value)} />
                    </div>
                  ))}
                  <div>
                    ...
                    <select className="cd-select" value={formData.fleetSize} onChange={e => updateForm("fleetSize", e.target.value)}>
                      ...
                      {[["1-4","1–4 Vehicles"],["5-9","5–9 Vehicles"],["10-24","10–24 Vehicles"],["25+","25+ Vehicles"]].map(([v,l]) => <option key={v} value={v}>{l}</option>)}
                    </select>
                  </div>
                  <div>
                  ...
                  <button className="cd-btn-gold" onClick={() => canSubmit && setSubmitted(true)} style={{ padding: "14px", fontSize: 12, borderRadius: 2, marginTop: 4, opacity: canSubmit ? 1 : 0.5 }}>
      ...
      <div style={{ background: `linear-gradient(135deg,${BRAND.navy},${BRAND.navyLight})`, padding: "68px 48px" }}>
        <div style={{ ...W, padding: 0, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", justifyContent: "flex-end" }}>
            <button className="cd-btn-gold" style={{ padding: "16px 36px", fontSize: 13, borderRadius: 2 }}>📞 Call Corporate Desk</button>
            <button className="cd-btn-outline" style={{ padding: "16px 36px", fontSize: 13, borderRadius: 2 }}>📧 Email Us</button>
          </div>
        </div>
      </div>
    </Layout>
