...
const PageStyles = () => (
  <style>{`
    ...
    .fin-btn-gold:disabled { opacity:0.6; cursor:not-allowed; }
    ...
    .fin-api-err { background:#fef2f2; color:#991b1b; border:1px solid #fca5a5; border-left:4px solid #ef4444; padding:14px 18px; border-radius:2px; margin-bottom:16px; font-size:13px; line-height:1.6; font-family:'Jost',sans-serif; }
  `}</style>
);
  ...
  <section style={{ minHeight:"62vh", background:"linear-gradient(135deg," + BRAND.navy + " 0%,#0e2a5c 55%," + BRAND.navyMid + " 100%)", display:"flex", alignItems:"center", position:"relative", overflow:"hidden" }}>
    ...
    <div style={{ ...W, paddingTop:80, paddingBottom:80 }}>
      ...
      <p className="fin-fadeUp" style={{ fontSize:17, lineHeight:1.8, color:"rgba(255,255,255,0.65)", maxWidth:540, marginBottom:40, animationDelay:"0.4s", opacity:0 }}>
        Partnered with leading national and local banks to bring you the lowest EMIs, quickest approvals, and most flexible repayment plans in North Karnataka.
      </p>
    ...
    <section style={{ background:"#ffffff", padding:"100px 0" }}>
      <div style={W}>
        <div style={{ textAlign:"center", marginBottom:56 }}>
          ...
          <p style={{ fontSize:15, color:BRAND.muted, marginTop:12 }}>Indicative rates — actual rate depends on vehicle, profile and tenure.</p>
        </div>
        <div style={{ display:"flex", justifyContent:"center", gap:0, marginBottom:48 }}>
          {[["national","National Banks"],["local","Local & Co-op Banks"]].map(([id, label]) => (
...
const loanServices = [
  ...
  { icon:"🔄", title:"Used Car Loan",             desc:"Refinance your existing vehicle or purchase a certified pre-owned car with our used-car finance option. Age and condition norms apply.", tag:"" },
...
const EMICalculator = () => {
  ...
  const fmtINR     = (n) => "Rs." + n.toLocaleString("en-IN");
  ...
  <section style={{ background:"#ffffff", padding:"100px 0" }}>
    <div style={W}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:0, position:"relative" }}>
        {[
          { step:"01", title:"Fill the Form",   desc:"Share your basic details using the enquiry form below.",               icon:"📝" },
          { step:"02", title:"Team Calls You",  desc:"Our finance advisor contacts you within 24 hours.",                    icon:"📞" },
          { step:"03", title:"Document Check",  desc:"Minimal KYC documents — Aadhaar, PAN, income proof.",                 icon:"📋" },
          { step:"04", title:"Bank Approval",   desc:"We apply to the most suitable lender for fastest approval.",           icon:"🏦" },
          { step:"05", title:"Drive Home!",     desc:"Loan disbursed, keys in hand — you drive away.",                      icon:"🚗" },
        ].map((s,i) => (
...
// ══════════════════════════════════════════════════════════════════
//  APPLY FORM — connected to /backend/api/loan-enquiry.php
// ══════════════════════════════════════════════════════════════════
const ApplyForm = () => {
  const EMPTY = { name:"", phone:"", email:"", city:"", vehicle:"", loanAmount:"", employmentType:"", message:"" };
  ...
  const validate = () => {
    ...
    if (!form.loanAmount)                          e.loanAmount  = "Please select a loan amount range.";
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await apiPost("loan-enquiry.php", form);
      setSubmitted(true);
    } catch (ex) {
  ...
  const loanAmounts = ["Up to Rs.3 Lakh","Rs.3-5 Lakh","Rs.5-8 Lakh","Rs.8-12 Lakh","Rs.12-18 Lakh","Rs.18-25 Lakh","Above Rs.25 Lakh"];
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1.6fr", gap:64, alignItems:"start" }}>
          ...
          <div style={{ paddingTop:8 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
              {[
                ["✅","Free consultation — no fees to apply"],
                ["🏦","Access to 10+ lenders in one place"],
                ["⚡","Approval in as fast as 24 hours"],
                ["🔒","Your data is safe and never shared"],
                ["📞","Dedicated RM assigned to your case"],
              ].map((item) => (
                <div key={item[1]} style={{ display:"flex", gap:14, alignItems:"flex-start" }}>
                  <span style={{ fontSize:18 }}>{item[0]}</span>
                  <span style={{ fontSize:14, color:"rgba(255,255,255,0.6)", lineHeight:1.6 }}>{item[1]}</span>
                </div>
              ))}
            </div>
          ...
          <div style={{ background:"rgba(255,255,255,0.97)", padding:"48px 40px" }}>
            {submitted ? (
              <div className="fin-success" style={{ textAlign:"center", padding:"40px 0" }}>
                ...
                <p style={{ fontSize:15, lineHeight:1.8, color:BRAND.muted, marginBottom:32 }}>
                  Thank you, <strong>{form.name}</strong>! Our finance team will call you at <strong>{form.phone}</strong> within 24 hours.
                </p>
                <button
                  className="fin-btn-gold"
                  style={{ padding:"13px 32px", fontSize:12, borderRadius:2 }}
                  onClick={() => { setSubmitted(false); setForm(EMPTY); setApiError(""); setErrors({}); }}
                >
              ...
              <>
                ...
                {apiError && (
                  <div className="fin-api-err">{apiError}</div>
                )}
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                  ...
                  <div>
                    ...
                    <input className="fin-input" placeholder="10-digit mobile" maxLength={10} value={form.phone}
                      onChange={(e) => { set("phone", e.target.value.replace(/\D/g, "")); setErrors((er) => ({ ...er, phone:"" })); }}
                  ...
                  <div>
                    ...
                    <input className="fin-input" placeholder="you@email.com" type="email" value={form.email}
                      onChange={(e) => { set("email", e.target.value); setErrors((er) => ({ ...er, email:"" })); }}
                      style={{ ...inputStyle, borderColor: errors.email ? "#e53e3e" : undefined }} />
                    {errors.email && <div style={errStyle}>{errors.email}</div>}
                  </div>

                  <div>
                    <label style={labelStyle}>City / Town</label>
                    <input className="fin-input" placeholder="e.g. Hubli, Gulbarga" value={form.city}
                    ...
                    <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                      {empTypes.map((t) => (
                        <button key={t} type="button"
                          onClick={() => set("employmentType", form.employmentType === t ? "" : t)}
                  ...
                  <div style={{ gridColumn:"1/-1" }}>
                    ...
                    <textarea className="fin-input" placeholder="Any specific requirement..." rows={3} value={form.message}
                ...
                <button
                  ...
                  disabled={loading}
                  style={{ width:"100%", marginTop:24, padding:"16px", fontSize:13, borderRadius:2 }}
                >
                  <span>{loading ? "Submitting..." : "Submit Enquiry — Our Team Will Call You"}</span>
                </button>
                <p style={{ fontSize:12, color:BRAND.muted, textAlign:"center", marginTop:16 }}>
